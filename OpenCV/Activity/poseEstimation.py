...
with np.load('calibration_data.npz') as file:
    mtx, dist, rvecs, tvecs = [file[i] for i in ('cameraMatrix','distCoeffs','rvecs','tvecs')]
...
for image in glob.glob('.\Images\Calib\\teste*.png'):
    ...
    if ret == True:
        ...
        k = cv.waitKey(0) & 0xFF
        if k == ord('s'):
            cv.imwrite('.\Images\Pose\\pose'+count+'.png', img)
